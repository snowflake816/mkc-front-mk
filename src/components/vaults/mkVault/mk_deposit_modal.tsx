...
import { depositToMkVault, depositToPEVault } from "../../../apis/backendAPI";
...
interface Props {
  ...
  setMkVaultTransactions: React.Dispatch<React.SetStateAction<any[]>>;
  apy: number;
}

const MkDepositModal = ({
  setDepositModal,
  setMkVaults,
  setMkVaultTransactions,
  apy
}: Props) => {
  ...
  const handleDeposit = async () => {
    ...
    setLoading(true);
    const rdata = await depositToMkVault(user.id, Number(amount));
    if (rdata.user) {
      setUser(rdata.user);
      if (rdata.user.mkVault) {
    ...
    toast.success(`Deposited ${amount} USD to MK Vault!`);
  };
