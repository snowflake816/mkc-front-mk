...
import { withdrawFromPEVault } from "../../../apis/backendAPI";
...
const PEWithdrawModal = ({
  ...
  const handleWithdraw = async () => {
    ...
    const rdata = await withdrawFromPEVault(user.id, vault, Number(withdrawAmount), feeAmount);
    if (rdata.user) {
      setUser(rdata.user);
      ...
      if (rdata.user.peVault.length == 0) {
        setVaultPlans(defaultVaultPlans);
      }
    }
    if (rdata.peVaultTransactions) {
      setVaultTransactions(rdata.peVaultTransactions);
    }
  ...
  useEffect(() => {
    changeAmount(currentData?.balance + currentData?.earning);
  }, [])
...
export default PEWithdrawModal;
