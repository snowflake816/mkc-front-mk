...
import { depositToPEVault } from "../../../apis/backendAPI";
...
interface Props {
  ...
  setPeVaultPlans: React.Dispatch<React.SetStateAction<any>>;
  setPeVaultTransactions: React.Dispatch<React.SetStateAction<any[]>>;
  vault: string;
  apy: number;
}

const PEDepositModal = ({
  vault,
  apy,
  setDepositModal,
  setPeVaultPlans,
  setPeVaultTransactions,
}: Props) => {
  ...
  const handleDeposit = async () => {
    ...
    if (Number(amount) < 500) {
      toast.info("Minimum Amount to deposit is 500 USD");
      return;
    }

    setLoading(true);
    const rdata = await depositToPEVault(user.id, vault, Number(amount));
    if (rdata.user) {
      setUser(rdata.user);
      if (rdata.user.peVault) {
        rdata.user.peVault.map((vault: any, index: number) => {
